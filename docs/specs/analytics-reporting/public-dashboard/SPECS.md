...
- **Site deletion** — Return `NOT_FOUND` for every public request while the Site is `deleting`, `deleted`, or `purged`; recovery restores the prior public configuration and identifier because Site deletion is not equivalent to `disablePublicDashboard`.
