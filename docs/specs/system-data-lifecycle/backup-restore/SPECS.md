...
```text
no operation -> creating -> available
creating -> failed
...
| --------------------------- | --------------- | --------------------------------------- |
| `id`                        | `SId`           | Backup operation/manifest identifier generated by Cimi's in-house ID generator. |
| `status`                    | `SBackupStatus` | `creating`, `available`, `restoring`, or `failed`. |
| `createdAt` / `completedAt` | `SDateTime`     | Operation timestamps.                   |
| `scope`                     | `installation` literal | `installation` data scope.              |
| `phase`                     | `SBackupPhase`  | SQLite capture/restore, DuckDB rebuild, cleanup, ready, or failed phase. |
| `progress`                  | `number`        | Monotonic operation progress from 0 through 1. |
| `checkpoint`                | `SBackupCheckpoint` | `none`, `sqlite_captured`, `sqlite_restored`, `duckdb_rebuilt`, or `structurally_ready`. |
| `lastSafeSequence`          | `integer` or `null` | SQLite acceptance sequence safe to resume or roll back from. |
| `readiness`                 | `SBackupReadiness` | Control-store, analytics-store, and structural readiness states. |
| `cleanupPending`            | `boolean`       | Historical retention/deletion cleanup remains after structural readiness. |
| `derivedCleanup` / `backupCleanup` | `SBackupCleanupStage` | Independent active-derived and historical-backup cleanup statuses, timestamps, and safe errors. |
| `restoreSourceBackupId`     | `SId` or `null` | Operator-selected backup manifest for a restore operation. |
| `preRestoreSafetyArtifact`  | `SPreRestoreSafetyArtifact` or `null` | Internal SQLite snapshot manifest used only to restore the pre-restore generation. |
| `errorCode`                 | `SBackupErrorCode` | Safe failure reason, if failed.         |
...
| --- | ----------------- | ------ | ------------------ | ----- | ------- |
| Q1  | `listBackups`     | GET    | `/backup-restore/listBackups`     | admin | query   |
| Q2  | `getBackupStatus` | GET    | `/backup-restore/getBackupStatus` | admin | query   |
| C1  | `createBackup`    | POST   | `/backup-restore/createBackup`    | admin | command |
| C2  | `restoreBackup`   | POST   | `/backup-restore/restoreBackup`   | admin | command |

## 4. Queries

### Q1: `GET /backup-restore/listBackups` — `listBackups`
...
### Q2: `GET /backup-restore/getBackupStatus` — `getBackupStatus`
...
### C1: `POST /backup-restore/createBackup` — `createBackup`
...
### C2: `POST /backup-restore/restoreBackup` — `restoreBackup`
