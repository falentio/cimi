...
- RPC procedures use explicit OpenAPI route metadata. Resource procedures use `/{resource}/{operation}` paths with kebab-case resource segments; `health` remains the `/system/health` operational-probe exception (contract path, served at `GET /api/system/health` under the runtime prefix). GET is for reads; POST is for ingestion, commands, and mutations.
...
- Cimi-generated entity IDs use the in-house prefixed generator from `@cimi/utils`. Contract boundaries validate IDs as opaque `SId` strings (1-128 characters); the generator's prefix and encoding are not API invariants. Generation rule (observability only): <3-letter>_<payload> with a lowercase three-letter prefix chosen for the entity type; clients must not parse or validate the prefix. A prefix change is not a breaking API change.
...
All first-release product specifications are `draft` until the corresponding contract schemas and handlers exist. This is intentional: planning is complete enough to implement without reopening domain decisions, but only the served routes listed in Status have runtime handlers; the remaining resources stay contract-only. Illustrative specifications under `examples/` may use their own implementation status.

A declared contract member is not a served API route. A procedure is available only after a runtime handler is registered and its authorization, persistence, lifecycle, transport, and error boundaries are implemented. At the current repository state, `apps/api` serves `health` at `GET /api/system/health`, the illustrative `hello` procedures, and the `installation`, `organization`, `membership`, `invitation`, `site`, `retention-policy`, `collection-policy`, `backup-restore`, `event-ingestion`, `identity-profile`, `traffic-report`, `event-report`, `goal`, `funnel`, `cohort-retention`, and `public-dashboard` resource procedures. `traffic-report` serves `getTrafficOverview` and `getTrafficBreakdowns`, and `event-report` serves `getEventOverview`, `getEventTimeseries`, `listEvents`, and `getEventBreakdowns`, all through the reporting admission pipeline against the DuckDB projection. The `exit_page` breakdown dimension derives each session's exit page as the last event with a page path over the session's full span, including events after the report window, and the `session.exitPage` filter compares against the same derived value. Two projection limits remain visible in the served surface: the `region` and `city` breakdown dimensions return an empty page with the real denominator because the projection cannot populate them, and an approved profile trait filter returns `BAD_REQUEST` because the projection carries no trait dimension. Contract `admission` annotations on reporting procedures do not imply served routes; they take effect as routers are registered. All served RPC routes sit under the `/api` runtime prefix. Update this status paragraph whenever route registration changes.
