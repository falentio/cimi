...
- Site deletion is recoverable and fail-closed: every non-`active` Site (`deleting`, `deleted`, `recovering`, or `purged`) is hidden and non-ingestible/non-queryable except for the privileged deletion-status surface where specified; Owners/Administrators may recover Sites for 30 days, after which live Site data is purged and backup copies follow normal retention without permitting resurrection.
