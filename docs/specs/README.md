...
A declared contract member is not a served API route. A procedure is available only after a runtime handler is registered and its authorization, persistence, lifecycle, transport, and error boundaries are implemented. At the current repository state, `apps/api` serves `health` at `GET /api/system/health`, the illustrative `hello` procedures, and the `installation`, `organization`, `membership`, `invitation`, `site`, `retention-policy`, `collection-policy`, `backup-restore`, `event-ingestion`, `identity-profile`, `traffic-report`, and `event-report` resource procedures. `traffic-report` serves `getTrafficOverview` and `getTrafficBreakdowns`, and `event-report` serves `getEventOverview`, `getEventTimeseries`, `listEvents`, and `getEventBreakdowns`, all through the reporting admission pipeline against the DuckDB projection. Two projection limits remain visible in the served surface: the `exit_page`, `region`, and `city` breakdown dimensions return an empty page with the real denominator because the projection cannot populate them, and an approved profile trait filter returns `BAD_REQUEST` because the projection carries no trait dimension. The remaining first-release resources are contract-only. They are `goal`, `funnel`, `cohort-retention`, and `public-dashboard`. Contract `admission` annotations on reporting procedures do not imply served routes; they take effect as routers are registered. All served RPC routes sit under the `/api` runtime prefix. Update this status paragraph whenever route registration changes.
