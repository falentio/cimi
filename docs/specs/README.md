...
A declared contract member is not a served API route. A procedure is available only after a runtime handler is registered and its authorization, persistence, lifecycle, transport, and error boundaries are implemented. At the current repository state, `apps/api` serves `health` at `GET /api/system/health`, the illustrative `hello` procedures, and the `installation`, `organization`, `membership`, `invitation`, `site`, `retention-policy`, `collection-policy`, `backup-restore`, `event-ingestion`, `identity-profile`, `traffic-report`, `goal`, `funnel`, and `cohort-retention` resource procedures. `traffic-report` serves `getTrafficOverview` and `getTrafficBreakdowns` through the reporting admission pipeline, but metric aggregation is not implemented: overview metrics are returned zero-filled with `complete: false` and breakdown pages are empty, so neither result states a measurement it does not have. The remaining first-release resources are contract-only. They are `event-report` and `public-dashboard`. Contract `admission` annotations on reporting procedures do not imply served routes; they take effect as routers are registered. All served RPC routes sit under the `/api` runtime prefix. Update this status paragraph whenever route registration changes.
