...
- Collection acknowledgment is independent from analytical materialization. Reports expose projected acceptance sequence, occurrence-time coverage, lifecycle freshness where relevant, and known Projection Gaps.
- A repeatedly failing accepted Event is quarantined and the projector advances. Later independent facts continue; affected stateful projections are degraded until the Event is replayed or the affected Site is rebuilt.
- Deletion is hidden immediately through a SQLite-canonical Deletion Tombstone in the live installation. Physical DuckDB cleanup is asynchronous and incremental, followed by compaction; a full rebuild remains the repair path. Restoring an older backup may temporarily expose payloads until cleanup catches up.
- Identity deletion uses a SQLite-canonical Identity Redaction overlay. Accepted Event sequence history remains immutable; replay and queries remove profile, alias, trait, and identity linkage, and retained non-personal Event facts may re-enter analytics as anonymous activity.
...
- Silent projection gaps: rejected because advancing after quarantine without reporting incompleteness would make analytical results appear complete when they are not.
...
- Quarantine preserves ingestion and unrelated analytical progress but requires a gap ledger, degraded report status, replay tooling, and Site-scoped rebuild support.
