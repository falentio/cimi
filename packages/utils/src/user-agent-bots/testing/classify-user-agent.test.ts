...
import { classifyUA, isBotUA } from '../index.ts'
...
describe('classifyUA', () => {
  ...
  it('reuses the cached classification for repeated UAs', () => {
    const userAgent = 'CacheProbeBot/1.0'
    expect(classifyUA(userAgent)).toBe(classifyUA(userAgent))
  })

  it('isBotUA boolean shorthand agrees with classifyUA', () => {
    expect(isBotUA('Googlebot/2.1')).toBe(true)
    expect(
      isBotUA('Mozilla/5.0 (Windows NT 10.0) AppleWebKit/537.36 Chrome/120 Safari/537.36'),
    ).toBe(false)
  })
})
