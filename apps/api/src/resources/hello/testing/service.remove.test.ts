...
describe('HelloService.remove', () => {
  it('removes an owned greeting and hides missing or inaccessible records', async () => {
    ...
    await expect(service.remove({ id: 'hel_1' }, { id: 'user_1' })).resolves.toEqual({
      id: 'hel_1',
    })
    expect(repo.deleteById).toHaveBeenCalledWith('hel_1', 'user_1')
    await expect(service.remove({ id: 'hel_1' }, { id: 'user_2' })).rejects.toMatchObject({
      code: 'NOT_FOUND',
    })
  ...
  it('rejects a removal when the atomic delete observes no row', async () => {
    ...
    await expect(service.remove({ id: 'hel_1' }, { id: 'user_1' })).rejects.toMatchObject({
      code: 'NOT_FOUND',
    })
  })
