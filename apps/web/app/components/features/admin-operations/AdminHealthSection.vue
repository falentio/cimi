...
<template>
  <section aria-labelledby="admin-health-title" class="space-y-3">
    <div>
      ...
      <p class="text-muted-foreground text-sm">Control and analytics readiness, separately.</p>
    </div>
    ...
    <Card v-else>
      <CardHeader>
        <CardTitle>
          <template v-if="report">Current health: {{ report.title }}</template>
          <template v-else>Documented health states</template>
        </CardTitle>
        <CardDescription>
          <template v-if="report">{{ report.description }}</template>
          <template v-else>
